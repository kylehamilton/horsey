...
horsey_build_req <- function(cl, path, query = NULL, accept = NULL) {
...
horsey_perform <- function(req, quiet = FALSE, max_tries = 2L) {
  tries <- 0L
  repeat {
    tries <- tries + 1L
    ...
    if (status == 429 && tries < max_tries) {
      if (!quiet) cli::cli_warn("Rate limited (HTTP 429). Waiting 60 seconds, then retrying once.")
      Sys.sleep(60)
      next
    }
    ...
    if (status >= 400) {
      ...
      rlang::abort(paste0("Lichess API error ", status, if (nzchar(body)) paste0(": ", body) else ""))
    }
...
horsey_parse_ndjson <- function(lines) {
  ...
  lapply(lines, function(line) jsonlite::fromJSON(line, simplifyVector = TRUE))
}
