...
user <- function(cl, username) {
  ...
  tibble::tibble(
    ...
    createdAt = as.POSIXct((dat$createdAt %||% NA_real_) / 1000, origin = "1970-01-01", tz = "UTC"),
    seenAt = as.POSIXct((dat$seenAt %||% NA_real_) / 1000, origin = "1970-01-01", tz = "UTC"),
...
rating_history <- function(cl, username) {
  req <- horsey_build_req(cl, paste0("/api/user/", username, "/rating-history"),
  ...
  for (perf in rh) {
    ...
    for (pt in points) {
      ...
      rows[[idx]] <- tibble::tibble(
        ...
        date = as.Date(sprintf("%04d-%02d-%02d",
                               as.integer(pt[[1]]),
                               as.integer(pt[[2]]) + 1L,
                               as.integer(pt[[3]])))
      )
  ...
  if (length(rows) == 0) {
    return(tibble::tibble(perf = character(), rating = integer(), date = as.Date(character())))
  }
